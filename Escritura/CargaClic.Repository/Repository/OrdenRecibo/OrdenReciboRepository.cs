    ...
    public class OrdenReciboRepository : IOrdenReciboRepository
    {
        ...
        public async Task<EquipoTransporte> assignmentOfDoor(long EquipoTransporteId, int UbicacionId)
        {
            using(var transaction = _context.Database.BeginTransaction())
            {
                ...
                equipoTransporteBd.EstadoId = (int) Constantes.EstadoEquipoTransporte.EnCola;
                ...
                {
                   item.EstadoId = (int) Constantes.EstadoEquipoTransporte.EnCola;
            ...
            using(var transaction = _context.Database.BeginTransaction())
            {
                    try
                    {
                            ...
                            for (int i = 0; i < interacciones ; i++)
                            {
                                    ...
                                    dominio.ClienteId = command.PropietarioId;
                                    ...
                                    // huelladetalle.UntQty es la cantidad de iteracciones.. si es una pallet
                                    ...
                                    // max  = 'E' + (Convert.ToInt64(max.Substring(1,8)) + i).ToString().PadLeft(8,'0');
                                    dominio.LodNum = "";
                            ...
                            var inventarios =   _context.InventarioGeneral.Where (x=>x.LineaId == linea.Id);
                            foreach (var item in inventarios)
                            {
                                item.LodNum =   'E' + (item.Id).ToString().PadLeft(8,'0');
                            }
                            await _context.SaveChangesAsync();
